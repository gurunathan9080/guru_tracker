...
def about_us():
    ...
    cards = [
        {"icon": "🚀", "title": "Our Mission", "desc": "Empower individuals globally with AI-driven financial insights to manage money smarter."},
        {"icon": "👩‍💻", "title": "The Team", "desc": "A dedicated group of fintech experts, AI researchers, and software engineers passionate about innovation."},
        {"icon": "🧠", "title": "Technology", "desc": "Leveraging state-of-the-art AI, ML, and blockchain technologies for next-generation finance tools."},
        {"icon": "🔒", "title": "Privacy Focus", "desc": "User data is secured with the highest standards, ensuring privacy and transparency at all times."},
        {"icon": "🌱", "title": "Sustainability", "desc": "Committed to promoting sustainable financial habits and environmental responsibility."},
        {"icon": "🤝", "title": "Community", "desc": "Building strong partnerships with users and experts to continuously improve and innovate."},
    ]
...
def user_profile_section():
    ...
    st.session_state.update({"income": inc, "goal": goal, "enable_crypto": enable_crypto, "track_sustainability": track_sustainability})
...
def add_expense_section():
    ...
    with st.form("expense_form"):
        ...
        category = ai_categorize(desc) if auto_cat and desc else st.selectbox("Select Category", list(st.session_state["budget"].keys()))
...
def budget_adjustment_section():
    ...
    for cat, val in st.session_state["budget"].items():
        new_budget[cat] = st.number_input(f"{cat} Budget ($)", min_value=0.0, value=float(val), format="%.2f", key=f"budget_{cat}")
...
def budget_vs_spent_section():
    ...
    this_month_exp = expenses[(pd.to_datetime(expenses["Date"]).dt.month == month) & (pd.to_datetime(expenses["Date"]).dt.year == year)]
    grouped = this_month_exp.groupby("Category")["Amount"].sum().reindex(st.session_state["budget"].keys(), fill_value=0)
    ...
    ], layout=go.Layout(barmode='group', title="Budget vs Spending This Month", xaxis_title="Category", yaxis_title="Amount ($)"))
...
def finance_chatbot_section():
    ...
    for sender, msg in st.session_state["chat_log"]:
        if sender == "You":
            st.markdown(f"<div style='background:black;padding:8px;border-radius:8px;'><b>You:</b> {msg}</div>", unsafe_allow_html=True)
        else:
            st.markdown(f"<div style='background:black;padding:8px;border-radius:8px;'><b>AI:</b> {msg}</div>", unsafe_allow_html=True)
# ------------- Main Function -------------
def main():
